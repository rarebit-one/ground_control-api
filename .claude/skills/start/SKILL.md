---
name: start
description: "Start working on Linear issues. Use when the user says 'start working on', 'pick up issue', 'work on GCL-123', or wants to begin development on Linear issues. Handles status updates, branch creation, and context gathering."
---
...
```bash
git stash push -m "WIP before starting GCL-123"
git fetch origin main
git checkout -b <branch-name> origin/main
```
...
```bash
git fetch origin main
git worktree add .worktrees/gcl-123 -b <branch-name> origin/main
```
...
`{identifier}/{short-description}` (e.g., `gcl-123/add-feature-name`)
